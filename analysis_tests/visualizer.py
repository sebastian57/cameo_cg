...
class LossPlotter:
    ...
    Example:
        >>> plotter = LossPlotter("train_12345.log", config)
        >>> plotter.parse_log()
    ...
    def _format_config_text(self) -> str:
        ...
        for i, stage in enumerate(stages):
            opt = stage["optimizer"]
            cfg = self.config.get_optimizer_config(opt)
            lines.append(f"Opt{i+1} ({opt}): lr={cfg.get('peak_lr', 'N/A')}")
