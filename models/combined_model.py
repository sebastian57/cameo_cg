...
class CombinedModel:
    ...
    def __init__(self, config, R0: jax.Array, box: jax.Array, species: jax.Array, N_max: int,
        ...
        Args:
            ...
            O0: Initial orientations, shape (n_atoms, 3,3) (optional)
        ...
        if "O0" in inspect.signature(ModelClass.__init__).parameters:
            ml_kwargs["O0"] = O0    
    ...
    def _support_gated_ml_energy(
        ...
        if (
            ...
            E_ml_raw = self.ml_energy_scale * self.ml_model.compute_energy(
                ml_params, R, mask, species, neighbor=neighbor, segment_id=segment_id, box=box, orientations=O
            )
            if not self.support_gate_enabled or self.support_gate_bank is None:
    ...
    def compute_energy(
        ...
        Args:
            ...
            box: Optional box vectors (optional)
            O: Optional orientation vectors (optional)
        ...
        E_ml, _, _ = self._support_gated_ml_energy(
            params['ml'], R, mask, species, neighbor, segment_id, box, O
        )
    ...
    def compute_total_energy(
        ...
        return self.compute_energy(
            params, R, mask, species, neighbor, segment_id=segment_id, box=box, O=O
        )
    ...
    def compute_teacher_projected_features(
        ...
        aux = self.compute_al_features(params, R, mask, species, neighbor, segment_id=segment_id, O=O)
    ...
    def compute_components(
        ...
        if self.prior_only:
            ...
            E_ml, E_ml_raw_scaled, support_alpha = self._support_gated_ml_energy(
                params['ml'], R, mask, species, neighbor, None, box, O
            )
    ...
    def energy_fn_template(self, params: Dict[str, Any]):
        ...
        def energy_fn(R: jax.Array, neighbor: Any, **kwargs) -> jax.Array:
            ...
            E = self.compute_energy(
                params, R, mask, species, neighbor=neighbor, segment_id=segment_id, box=box, O=O
            )
            return E
