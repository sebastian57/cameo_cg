...
for i in range(n_rep):
    a = np.load(d / f"eqtest_ref_rep{i:02d}.npz")["R"].astype(np.float64)
    b = np.load(d / f"eqtest_vmap_rep{i:02d}.npz")["R"].astype(np.float64)
    if a.shape != b.shape:
    ...
    d0 = np.abs(a[0] - b[0]).max()
    tol = 1e-4 * max(np.abs(a[0]).max(), 1.0)   # float32 eps ~1.2e-7, 100 steps of accumulation
    if not np.isfinite(d0) or d0 > tol:
        fails.append(f"rep{i} frame0 max|dR|={d0:.3e} > tol={tol:.3e}")
    print("-" * 50)

if fails:
    print("\nFAIL:"); [print("  " + f) for f in fails]; sys.exit(1)
print("\nPASS: batched route reproduces MDRunner to float32 precision "
      "at the first recorded frame for all replicas.")
