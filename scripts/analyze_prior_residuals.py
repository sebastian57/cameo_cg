...
def _copy_config(config: ConfigManager) -> ConfigManager:
    ...
    copied._config = copy.deepcopy(config._config)
    return copied
...
def _resolve_spline_path_if_needed(config: ConfigManager) -> None:
    ...
    config._config.setdefault("model", {}).setdefault("priors", {})["spline_file"] = str(
        spline_path.resolve()
    )
...
def _ensure_residual_analysis_enabled(
    ...
    *,
    ...
    training_cfg = config._config.setdefault("training", {})
    residual_cfg = training_cfg.setdefault("prior_residual", {})
