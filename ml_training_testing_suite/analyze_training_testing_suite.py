#!/usr/bin/env python3
"""
Summarize dated ML training test runs into per-input analysis directories.

Training run layout:
    ml_training_testing_suite/outputs/YYYYMMDD_run-name/
        config_input.yaml
        config_runtime.yaml
        train_allegro_<jobid>.log
        slurm-<jobid>.out
        exports/
        checkpoints/

Analysis layout:
    ml_training_testing_suite/outputs/YYYYMMDD_run-name_analysis/
        summary.csv
        tail_loss_plots/
        force_eval_plots/

    ml_training_testing_suite/outputs/YYYYMMDD_training_testing_example_name_analysis/
        summary.csv
        tail_loss_plots/
        force_eval_plots/
"""
    ...
    if spline_path.is_absolute():
        ...
        for candidate in candidates:
            if candidate.exists():
                resolved = candidate
                break
    config._config.setdefault('model', {}).setdefault('priors', {})['spline_file'] = str(resolved.resolve())
...
def _collect_force_eval_data(
    ...
    *,
    ...
    config._config['model']['use_priors'] = bool(config.export_combined_ml_priors_enabled())
...
def _completed_run_reason(run_dir: Path, include_incomplete: bool = False) -> tuple[bool, str]:
    log_path = _latest_file(run_dir, 'train_allegro_*.log')
...
def _run_detailed_force_eval_subprocess(
    ...
    *,
    ...
    script_path = SUITE_ROOT / 'detailed_force_eval.py'
