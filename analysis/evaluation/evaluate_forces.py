...
from analysis.common.paths import repo_root
REPO_ROOT = repo_root()
...
from analysis.evaluation.visualizer import ForceAnalyzer
from analysis.evaluation.per_residue import (
...
def main():
    ...
    if config.use_spline_priors_enabled():
        ...
        config.set('model', 'priors', 'spline_file', abs_spline)
    ...
    if args.mode == 'prior-only':
        ...
        config.set('model', 'use_priors', bool(use_combined and has_prior_config))
        config.set('model', 'train_priors', False)
        print(
    ...
    if args.mode == 'prior-only':
        ...
        # for Evaluator compatibility (it expects params['ml'] to exist)
        if 'allegro' not in params:
            # Use a minimal dummy dict instead of full initialization
            params['ml'] = {'dummy': jnp.array(0.0)}
