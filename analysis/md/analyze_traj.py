...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--tica-per-file",
        action="store_true",
        help="Fit TICA per input file (no lagged pairs across trajectory seams). "
             "Use whenever inputs are many short independent replicas.",
    )
...
def fit_tica_segments(segments, X_all, lagtime):
    """TICA fitted on a LIST of trajectories, so lagged pairs never cross a seam.

    Concatenating independent replicas and fitting on the result (the default
    path) forms lagged pairs between the end of one replica and the start of the
    next. With few long trajectories that is negligible; with hundreds of short
    ones it is not (512 x 101 frames = 511 seams, ~10% of pairs at lagtime 10).
    deeptime's TICA accepts a list and handles the boundaries correctly.
    """
    try:
        from deeptime.decomposition import TICA
    except ImportError as exc:
    ...
    if lagtime < 1:
        raise ValueError("TICA lagtime must be >= 1")
    usable = [s for s in segments if s.shape[0] > lagtime]
    if not usable:
        raise ValueError(
            f"No input trajectory is longer than lagtime={lagtime} "
            f"(lengths {[int(s.shape[0]) for s in segments[:5]]}...)")
    dropped = len(segments) - len(usable)
    model = TICA(lagtime=lagtime, dim=2).fit(usable).fetch_model()
    Y = np.asarray(model.transform(X_all), dtype=np.float64)
    return model, Y, dropped
...
def main() -> None:
    ...
    file_lengths = [int(c.shape[0]) for c in all_coords]
    ...
    elif args.method == "tica" and args.tica_per_file:
        bounds = np.cumsum([0] + file_lengths)
        segs = [X[bounds[i]:bounds[i + 1]] for i in range(len(file_lengths))]
        model, Y, dropped = fit_tica_segments(segs, X, lagtime=args.lagtime)
        print(f"Fitted TICA (lagtime={args.lagtime}) on {len(segs) - dropped} "
              f"separate trajectories ({n_frames_total} frames, no cross-seam pairs"
              + (f"; {dropped} shorter than lagtime skipped" if dropped else "") + ")")
