...
def multidir_group_script(*, case_dirs: Sequence[str], structure_for: Sequence[str],
                          ...
                          index: str | None = None, plumed: bool = True,
                          trim: bool = False) -> str:
    ...
    for d, s in zip(case_dirs, structure_for):
        grompp_lines += [f'grompp_one "{d}" "{s}" &', "grompp_pids+=($!)"]
    grompp_lines += [
        'grompp_fail=0; for p in "' + chr(36) + '{grompp_pids[@]}"; do wait "$p" || grompp_fail=1; done',
        '[ "$grompp_fail" -eq 0 ] || { echo "a grompp failed" >&2; exit 1; }',
    ]
    grompp = "\n".join(grompp_lines)
...
"""
    ...
    # INODE TRIM. A stage-3 campaign creates ~11 inodes per state (1 dir + 10 files); at
    # 250,000 states that is ~2.75M, and on 2026-08-18 it exhausted the SHARED 4.0M inode
    # quota on /e/project1/cameo mid-run -- tasks 81-83 died with "File input/output error:
    # biased.log" and everything after could not even create a .out file.
    # What must survive: biased.trr (the actual forces) and unbiased_forces.trr, which with
    # plumed=False is a SYMLINK to it -- deleting biased.trr would destroy the data. seed.gro
    # and production.mdp stay so a state can be re-grompp'd or its realized target re-read.
    # Everything else is reconstructible or already consumed by the time mdrun returns.
    trim_block = "" if not trim else "\n".join([
        "",
        "# ---- 4. trim disposable per-state files (inode hygiene; see launch.py) -------",
        f'for d in {dirs}; do',
        '    rm -f "$d"/biased.cpt "$d"/biased.edr "$d"/biased.log "$d"/biased.xtc \\',
        '          "$d"/biased.gro "$d"/biased.tpr "$d"/mdout.mdp "$d"/step*.pdb',
        "done",
    ])
...
def submit_script(*, campaign_dir: Path, groups: Sequence[Sequence[int]], job_name: str,
    ...
    array_end = (len(groups) + groups_per_task - 1) // groups_per_task - 1
    array_limit = f"%{max_concurrent_tasks}" if max_concurrent_tasks is not None else ""
    return f"""#!/bin/bash
...
#SBATCH --array=0-{array_end}{array_limit}
...
for k in $(seq "$start" $(( start + PER - 1 ))); do
    [ "$k" -ge "$NG" ] && break
    echo "=== group $k ==="
    bash "{campaign_dir}/run_group_$(printf '%04d' "$k").sh"
done
"""
